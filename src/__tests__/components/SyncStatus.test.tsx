...
describe("SyncStatus", () => {
  ...
  it("returns null when cloud is not enabled", () => {
    ...
    const { container } = render(<SyncStatus sync={makeSyncProp()} isAuthenticated={true} />);
    expect(container.innerHTML).toBe("");
  });

  it("returns null when not authenticated", () => {
    const { container } = render(<SyncStatus sync={makeSyncProp()} isAuthenticated={false} />);
    expect(container.innerHTML).toBe("");
  });
  ...
  it("shows error with retry button", () => {
    render(<SyncStatus sync={makeSyncProp({ status: "error" })} isAuthenticated={true} />);
  ...
  it("calls triggerSync on button click", async () => {
    ...
    render(
      <SyncStatus sync={makeSyncProp({ status: "idle", triggerSync })} isAuthenticated={true} />
    );
  ...
  it("shows offline status", () => {
    render(<SyncStatus sync={makeSyncProp({ status: "offline" })} isAuthenticated={true} />);
